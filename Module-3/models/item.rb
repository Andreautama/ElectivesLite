...
class Item
    ...
    def save
        ...
        categories.each do |category|
            ...
            query = "Insert into item_category(item_id, category_id) select item_id , #{category} from items where name = '#{name}'"
        client.query(query)
        end
    end
    ...
    #     client = create_db_client
    #     raw_data = client.query("select c.category_id from item_category i_c join categories c on i_c.category_id = c.category_id")
    #     categories = []
    #     raw_data.each do |data| 
